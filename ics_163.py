...
def get_travel(t):
    dcc = []
    m_c = re.compile("\w\d*次").search(t)  # 查找车次信息
    # 获取车次网页信息
...
def run_ing(user, password, pop3_server):
    ...
    for i in range(index, 0, -1):
        ...
        if From == mail_12306:
            t = msgBodyContents[0].split('\r\n')[7]
            mail_list.append(t)

    for j in mail_list:
        # 打印车次信息
        print("ID:", mail_list.index(j), j.split('.')[1:][0])

    print('请选择要生成日历事件的订单ID')
    t_id = int(input())
    make_ics(mail_list[t_id])
